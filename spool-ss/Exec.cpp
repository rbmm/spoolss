...
NTSTATUS ExecAsUser(HANDLE hToken, PVOID BaseOfImage, ULONG size, PCWSTR lpCmdLine)
{
	...
	NTSTATUS status = RtlImageNtHeaderEx(0, BaseOfImage, size, &pinth);
	...
	if (0 <= (status = FindNoCfgDll(pinth->FileHeader.Machine, pinth->OptionalHeader.Magic, SizeOfImage, 0, &hSection)))
	{
		SIZE_T ViewSize = 0;
		PVOID BaseAddress = 0;
			...
			&BaseAddress, 0, 0, 0, &ViewSize, ViewUnmap, 0, PAGE_NOACCESS)))
		{
			...
			if (0 <= (status = ZwProtectVirtualMemory(NtCurrentProcess(), &pv, &RegionSize, PAGE_READWRITE, &op)))
			{
				...
				if (CreateProcessAsUserW(hToken, buf, const_cast<PWSTR>(lpCmdLine), 0, 0, 0, CREATE_SUSPENDED, 0, 0, &si, &pi))
				{
					...
					if (0 <= (status = NtQueryInformationProcess(pi.hProcess, ProcessBasicInformation, &pbi, sizeof(pbi), 0)) &&
						0 <= (status = NtQueryInformationProcess(pi.hProcess, ProcessWow64Information, &wow, sizeof(wow), 0)) &&
						0 <= (status = GetCtx(pi.hThread, &ctx)) &&
						0 <= (status = ZwMapViewOfSection(hSection, pi.hProcess, &RemoteBase, 0, 0, 0, &ViewSize, ViewShare, 0, PAGE_READONLY)))
					{
						...
						0 <= (status = ZwProtectVirtualMemory(pi.hProcess, &(pv = RemoteBase), &(RegionSize = SizeOfImage), PAGE_READWRITE, &op)) &&
							0 <= (status = ZwWriteVirtualMemory(pi.hProcess, RemoteBase, BaseAddress, SizeOfImage, 0)) &&
							0 <= (status = ProtectImage(pi.hProcess, RemoteBase, pinth)) &&
							0 <= (status = ZwWriteVirtualMemory(pi.hProcess, &reinterpret_cast<PEB*>(pbi.PebBaseAddress)->ImageBaseAddress, &RemoteBase, sizeof(RemoteBase), 0)) &&
							0 <= (status = (wow ? ZwWriteVirtualMemory(pi.hProcess, &wow->ImageBaseAddress, &RemoteBase, sizeof(ULONG), 0) : 0)) &&
							0 <= (status = SetCtx(pi.hThread, &ctx)) &&
							0 <= (status = ZwResumeThread(pi.hThread, 0));
					}
